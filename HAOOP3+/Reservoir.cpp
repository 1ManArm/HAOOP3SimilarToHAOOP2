...
const std::string Reservoir::filename_for_save = "Reservoir.bin";

Reservoir::Reservoir(std::string name, std::string type, float width, float height, float max_depth)
{
	reservoir_name = name;
	reservoir_type = type;
	this->width = width;
...
bool Reservoir::SetName(std::string name)
{
	if (name.size() > 0)
	{
		reservoir_name = name;
...
bool Reservoir::SetType(std::string name)
{
...
bool Reservoir::GetType(std::string name) const
{
...
void Reservoir::ShowAllParams() const {
	std::cout << "Название водоема: [" << reservoir_name << "] Тип [" << reservoir_type << "] Ширина: [" << width << "] Длина: [" << height << "] Глубина: [" << max_depth << "] Площадь водоема: [" << RevervoirSquare() << "] Обьем: [" << ReservoirVolume() << "]";

}

std::string Reservoir::GetFileName()
{
	return filename_for_save;
}

void Reservoir::Save(std::ofstream& of) {
...
void Reservoir::Load(std::ifstream& load) {
